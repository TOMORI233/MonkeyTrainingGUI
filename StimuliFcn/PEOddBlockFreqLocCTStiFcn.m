function PEOddBlockFreqLocCTStiFcn(device)
...
for trialN = 1 : sweepCountMax*2
    ...
    if prod([mod(trialN-1,blockTransNum(end)) - [0 blockTransNum(1)]]) <= 0 % in the range of freq
        cueType = 'freq';
    elseif prod([mod(trialN-1,blockTransNum(end)) - [blockTransNum(1)+1 blockTransNum(2)]]) <= 0 % in the range of location
        cueType = 'location';
    elseif prod([mod(trialN-1,blockTransNum(end)) - [blockTransNum(2)+1 blockTransNum(3)]]) <= 0 % in the range of rand
