function PEOddWoringMemoryMultiToneStiFcn(device)
...
% set freqPool and ratioControlPool
freqPool = [Freq1; Freq2; Freq3; Freq4];
ratioControlPool = [ratioControl1; ratioControl2; ratioControl3; ratioControl4];
fPropPool = [fProp1; fProp2; fProp3; fProp4];
...
if ~trialStartFlag && pushAfterDelayFlag && tCount >= pushTime + pushToOnsetInterval / period
    ...
    % std freq
    frequencyStd = randsrc(1, 1, [freqPool, ratioControlPool]');
    freqIdx = ismember(freqPool, frequencyStd);
    % std location
    locationStd = 0;
    ...
    % dev freq
    frequencyDev = randsrc(1, 1, [freqPool'; fPropPool(freqIdx, :)]);
