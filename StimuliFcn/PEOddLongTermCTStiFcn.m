function PEOddLongTermCTStiFcn(device)
...
for trialN = 1:sweepCountMax*2
    ...
    if mod(trialN-1,transTrialNum) == 0
        ...
        while any(freqRandPool < lowHighFreqRange(1) | freqRandPool > lowHighFreqRange(2)) % all random frequency should in lowHighLimitRange
            freqRandPool = normrnd(freqStdCopy , standardDeviation * freqStdCopy, [transTrialNum , 1]);
        end
        freqRandPool = ceil(freqRandPool);
    ...
    if transFlag
        frequencyStd = freqRandPool(mod(trialN - 1,transTrialNum) + 1);
    else
        frequencyStd = freqStdCopy;
    end
    % difference level
